...
//     }))
// );
const prevUrl = `/christiancommunity/position/${props.profile.position.toLowerCase()}`;
const form = useForm({
    id: props.profile?.id || "",
    ...
    redirectUrl: `/christiancommunity/position/${props.profile.position}`,
});
...
const onSelectedDownline = (opt) => {
    ...
    newDownlineform.municipality = voter.municipality_name;
...
const deleteChildData = () => {
    ...
    deleteDownlineform.delete(route('christiancommunity.bulkdelete'), {
        onSuccess: () => {
            closeModal();
            router.visit('', {
                only: ['props']
...
watch(
    ...
    (profile) => {
        ...
        if (profile) {
            ...
            form.redirectUrl = `/christiancommunity/position/${props.position}`;
        }
    }
);
...
watch(
    ...
    debounce(() => {
        router.get(
            "",
            { searchname: searchNameQuery.value || newDownline.value, filtermunicipality: props.profile.municipality, filterbarangay: props.profile.barangay },
...
const addDownline = () => {
    // console.log("test");
    newDownlineform.post(route("christiancommunity.adddownline"), {
        onSuccess: () => {
            router.visit('', {
                only: ['props']
            })
        ...
        <div class="max-w-full mx-auto py-4">
            ...
            <div class="bg-white overflow-hidden md:flex justify-evenly mt-12 md:gap-4">
                <div class="md:w-1/3 ml-12">
                    ...
                    <div class="mt-6">
                        <InputLabel for="municipality" value="Municipality" class="text-xl" />

                        <div class="mt-1 px-2 py-3 min-h-12 border border-gray-300 rounded-lg text-xl">{{
                            profile.municipality }}</div>
                    </div>
                    <div class="mt-6">
